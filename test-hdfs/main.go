...
func testConnection(fs *webhdfs.FileSystem) {
	_, err := fs.ListStatus(webhdfs.Path{Name: "/"})
	if err != nil {
...
func ls(fs *webhdfs.FileSystem, hdfsPath string) {
	stats, err := fs.ListStatus(webhdfs.Path{Name: hdfsPath})
	if err != nil {
...
func createTestDir(fs *webhdfs.FileSystem, hdfsPath string) {
	path := webhdfs.Path{Name: hdfsPath}
...
func uploadTestFile(fs *webhdfs.FileSystem, testFile, hdfsPath string) string {
...
func renameRemoteFile(fs *webhdfs.FileSystem, oldName, newName string) {
	_, err := fs.Rename(webhdfs.Path{Name: oldName}, webhdfs.Path{Name: newName})
	if err != nil {
...
func changeOwner(fs *webhdfs.FileSystem, hdfsPath string) {
	shell := webhdfs.FsShell{FileSystem: fs}
	_, err := shell.Chown([]string{hdfsPath}, "owner2")
	if err != nil {
		log.Fatal("Chown failed for ", hdfsPath, ": ", err.Error())
	}
	stat, err := fs.GetFileStatus(webhdfs.Path{Name: hdfsPath})
	if err != nil {
...
func changeGroup(fs *webhdfs.FileSystem, hdfsPath string) {
	shell := webhdfs.FsShell{FileSystem: fs}
...
func changeMod(fs *webhdfs.FileSystem, hdfsPath string) {
	shell := webhdfs.FsShell{FileSystem: fs}
...
func appendToRemoteFile(fs *webhdfs.FileSystem, localFile, hdfsPath string) {
	stat, err := fs.GetFileStatus(webhdfs.Path{Name: hdfsPath})
	if err != nil {
	...
	stat2, err := fs.GetFileStatus(webhdfs.Path{Name: hdfsPath})
	if err != nil {
...
func moveRemoteFileLocal(fs *webhdfs.FileSystem, remoteFile string) {
	...
	_, err = fs.GetFileStatus(webhdfs.Path{Name: remoteFile})
	if err == nil {
